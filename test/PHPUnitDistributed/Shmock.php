...
* {
*	$shmock->set_user($user)->once()->return_true();
* });
*
	...
	*/
	...
	protected function construct_mock()
	{
		...
		if ($this->preserve_original_methods)
		{
			...
			{
				/*
				 * The PHPUnit mock builder does not preserve any methods if none are mocked
				 * Mock a fake method to force the preservation of other methods on the mocked object
				 */
	...
	{
		$err_msg = "Attempted to expect #$method, which is a static method in the {$this->class} class. If you wish to disable this check, call \$shmock->disable_strict_method_checking()";
		try
		{
